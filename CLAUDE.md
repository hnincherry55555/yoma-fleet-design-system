...
1. `src/design/tokens.json` - raw Figma export (Variable Visualizer), the only tracked source
2. `scripts/generate-tailwind-theme.ts` - transformer
3. `src/design/tokens.css` - CSS custom properties (primitives + Light/Dark theme), generated
4. `src/design/theme.css` - Tailwind v4 `@theme inline` block, generated
5. `src/app/globals.css` - imports tokens.css and theme.css

Run `npm run sync-tokens` after updating tokens.json.

`tokens.css` and `theme.css` are build artifacts: gitignored, never committed, and
regenerated by the `prebuild` and `predev` hooks. So `npm run dev`, `npm run build`,
and every Vercel deploy derive them from `tokens.json` automatically. Do not add them
back to git, and do not hand-edit them.
